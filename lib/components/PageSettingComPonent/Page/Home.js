...
var PageSettingComPonent = function (_React$Component) {
    ...
    function PageSettingComPonent(props) {
                            ...
                            // 图文导航
                                ...
                                // 图文导航
                                    ...
                                    // 优惠券
                                            ...
                                            arr.push(_react2.default.createElement(_Coupons2.default, { item: item, history: _this.props.history, authIndex: index,
        ...
        _this.authorizationFailurePageSetting = function (authIndex) {
            _this.setState({
                authIndex: authIndex
            }, function () {
        ...
        _this.loginSuccess = function (data) {
            var authIndex = _this.state.authIndex;

            _this.hideLoginModal();
            localStorage.setItem('userInfo', (0, _stringify2.default)(data));
            // 防止数据发送太快导致会多次进入子组件的判断
            // setTimeout(() => {
            _this.props.dispatch({ type: 'pageSetting/commonRequest', payload: { guid: Math.random(), authIndex: authIndex } });
            // }, 1000);
        };
