...
require('./less/templateModal.less');
...
var TemplateModal = function (_React$Component) {
  ...
  function TemplateModal(props) {
    ...
    _this.getTemplate = function (id) {
      // 如果TemplateID不存在则不查询
      if (id) {
        _this.props.dispatch({ type: 'exchange/GetProductTemp', payload: { productTemplateId: id } });
      }
    };
        ...
        if (!err) {
          ...
          if ('ChargeAccount' in values) {
            ...
            var _this$props = _this.props,
                validType = _this$props.validType,
                inputTips = _this$props.inputTips;

            var _validTxt = '',
                validText = "";
            if (validType === 1) {
              //手机
              _validTxt = /^1(3|4|5|6|7|8|9)\d{9}$/;
              validText = '手机号';
            } else if (validType === 2) {
              // qq
              _validTxt = /^[1-9][0-9]{4,10}$/;
              validText = 'QQ号';
            } else if (validType === 3) {
              // 邮箱
              _validTxt = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/;
              validText = '邮箱';
            }
            if (!postData.ChargeAccount) {
              return _toast2.default.info('\u8BF7\u8F93\u5165\u6B63\u786E\u5145\u503C\u8D26\u53F7');
            }
            if (_validTxt && !_validTxt.test(postData.ChargeAccount)) {
              return _toast2.default.info('' + (inputTips && inputTips || '请输入正确的充值账号'));
            }
          }
  ...
  (0, _createClass3.default)(TemplateModal, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      var _nextProps$exchange = nextProps.exchange,
          getGameProTemp = _nextProps$exchange.getGameProTemp,
          toOrderMid = _nextProps$exchange.toOrderMid;
      ...
      if (nextProps.pageSetting.guid !== this.props.pageSetting.guid && this.props.componentIndex === nextProps.pageSetting.componentIndex) {
        ...
        var authType = nextProps.authType;
        // 卡密订单、直充订单
          ...
          inputs && inputs.length ? inputs.map(function (v) {
            if (v.type === 'Input') {
              return _react2.default.createElement(
                'div',
                { className: 'd-account' },
                _react2.default.createElement(_inputItem2.default, (0, _extends3.default)({}, getFieldProps(v.id), {
        ...
        choseProduct.isActivity ? _react2.default.createElement(
          ...
          choseProduct.activityState == 1 && passCodeStatus ? _react2.default.createElement(
            ...
            passCodeStatus.userPassCodeStatus === 4 && !isLoading ? _react2.default.createElement(
              _button2.default,
              { loading: this.props.exchangloading, className: 'btn-block prim-btn', onClick: this.exchange },
              '\u7ACB\u5373\u5151\u6362'
            ) : '',
